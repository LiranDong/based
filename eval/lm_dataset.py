# Originally from https://github.com/NVIDIA/Megatron-LM/blob/main/tasks/zeroshot_gpt/datasets.py
# Adapted from train/datamodules/datasets/lm_dataset.py in the Based codebase.
# We don't pad the last block and don't use overlapping eval.
# Returns both the input and the target.
...
class LMDataset(torch.utils.data.Dataset):
    """Language modeling dataset that returns (input, target) pairs.

    Args:
        tokens: numpy array of token IDs.
        seq_len: sequence length for each sample.
        drop_last: if True, drop the last incomplete block.
    """
